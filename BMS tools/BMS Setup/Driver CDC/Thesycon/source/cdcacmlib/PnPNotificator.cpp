...
// Window class name registered by CPnPNotificator.
// Note: this name is application local and should not cause conflicts with other applications.
static const char sWndClassName[] = "PnPNotificatorWndCls";

// title of the hidden window
static const char sWndTitle[] = "PnPNotificator_HiddenWindow";
...
//
bool
CPnPNotificator::RegisterAndCreateHiddenWindow(void)
{
  ...
  wcex.lpszClassName  = sWndClassName;
  //wcex.hIconSm    = NULL;
  ...
  if ( mhWnd==NULL ) {
    ...
    // unregister class
    ::UnregisterClass(sWndClassName, mWndOwner);
    return false;
  }


	//Warning C4244 ('argument' : conversion from 'type1' to 'type2', possible loss of data)
	//apparently appears due to the fact that the warning system operates on the code after 
	//it has been preprocessed and that SetWindowLongPtr is converted to SetWindowLong on 
	//32-bit Windows. The warning system detects a 64-bit portability problem. We know this
	//is no error and disable the warning for this piece of code. 
#pragma warning( push )
#pragma warning( disable : 4244 )
	// The window has been successfully created, attach this object
	SetLastError(0);
	if ( (0 == ::SetWindowLongPtr(mhWnd,GWLP_USERDATA,(LONG_PTR)this)) && (GetLastError() != 0) ) {
		::UnregisterClass(sWndClassName, mWndOwner);
		return false;
	}
#pragma warning( push )
...
//
bool
CPnPNotificator::UnregisterWindowClass(void)
{
  bool succ = true;

  // unregister the window class
  if ( !::UnregisterClass(sWndClassName, mWndOwner) ) {
    // error
    succ = false;
  }
  ...
  )
{
  ...
  if ( Notification->hDevNotify != NULL ) {
    ...
    // The following is a work-around for Win98:
    // Win98 becomes unstable after a call to UnregisterDeviceNotification.
    // So we don't call this function at all. Active registrations should
    // be deleted when the application exits.
    // Note: we use _winver provided by C runtime (stdlib.h) to detect the OS version.
		OSVERSIONINFO OsVersion;
		OsVersion.dwOSVersionInfoSize = sizeof(OsVersion);
		GetVersionEx(&OsVersion);

    if ( OsVersion.dwMajorVersion >= 0x5) { 
...
CPnPNotificator::ThreadRoutine(void)
{
  ...
  bool succ = RegisterAndCreateHiddenWindow();
  ...
  )
{
  LRESULT retcode = 0;
	...
	//after it has been preprocessed and that GetWindowLongPtr is converted
	//to GetWindowLong on 32-bit Windows. The warning system detects a 64-bit 
	//portability problem because LONG cannot be converted to a 64-bit pointer. 
	//We know this is no error and disable the warning for this piece of code. 
#pragma warning( push )
#pragma warning( disable : 4312 )
	// retrieve the pointer to our CPnPNotificator instance
	CPnPNotificator* This = (CPnPNotificator*)::GetWindowLongPtr(hWnd,GWLP_USERDATA);
#pragma warning( pop ) 
